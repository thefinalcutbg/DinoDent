...
void DbProcedure::saveProcedures(long long amblist_rowid, const std::vector<Procedure>& mList, Db& db)
{
	...
	for (int i = 0; i < mList.size(); i++)
	{
		...
		db.newStatement(
			...
			"(date, nzok, type, code, tooth, deciduous, data, ksmp, amblist_rowid, name, diagnosis, notes) "
			"VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
...
{
		std::string query =
		"SELECT  procedure.date, procedure.code, procedure.nzok, procedure.data, amblist.lpk, procedure.deciduous, procedure.ksmp, procedure.name, procedure.diagnosis, procedure.notes FROM "
