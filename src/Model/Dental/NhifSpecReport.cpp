...
const std::map<int, std::string> nhif_names
{
	...
	{ 834, { "Изработка на медицинско изделие тотална горна цяла плакова зъбна протеза"}},
	{ 835, { "Изработка на медицинско изделие тотална долна цяла плакова зъбна протеза"}},
...
PlainTable NhifSpecReport::getSpecificationReport() const
{
	...
	for (auto& [code, count] : procedures_minor) {
		...
		if (!nhif_names.count(code)) continue;
