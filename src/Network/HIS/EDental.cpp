...
void EDental::Open::parseReply(const std::string& reply)
{
	...
	for (int i = 3; contents.Child(i).ToElement(); i++)
	{
		...
		sequence -= 1; //compensating for the first procedure
#endif
...
bool EDental::Augment::sendRequest(const AmbList& ambSheet, const Patient& patient, decltype(m_callback) callback)
{
	m_callback = callback;
