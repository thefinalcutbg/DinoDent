...
inline QPixmap getSurfaceTexture(const ToothPaintHint& tooth)
{
    ...
    painter_of_united_surfaces.setOpacity(0.3);
...
inline void drawMobilityLabel(const ToothPaintHint& tooth, QPixmap& pixmap)
{
	...
	int yPos = tooth.idx > 15 ? 60 : pixmap.height() - 60;
...
inline void drawFractureLabel(const ToothPaintHint& tooth, QPixmap& pixmap)
{
	...
	int yPos = tooth.idx > 15 ? pixmap.height() - 50 : 50;
...
QPixmap* getBuccalOcclusal(const ToothPaintHint& tooth)
{
    ...
    QPixmap tooth = getToothPixmap(tooth);
    ...
    if (tooth.impacted)
    {
        ...
        painter.drawPixmap(QRect(0, yPos, coords.toothCrop.width(), 320), getToothPixmap(tooth), coords.buccalImpactCrop);
    }
    else
    {
...
QPixmap* getBuccalLingual(const ToothPaintHint& tooth)
{
    ...
    if (tooth.impacted)
    {
        ...
        int yPos = tooth.idx > 15 ? 200 : 40;
        painter.drawPixmap(QRect(0, yPos, coords.toothCrop.width(), 320), normalTooth, coords.buccalImpactCrop);
        painter.drawPixmap(QRect(0, 900, coords.toothCrop.width(), 320), normalTooth, coords.lingualImpactCrop);
    }
    else
    {
        painter.drawPixmap(QRect(0, toothYPosition,
            coords.toothCrop.width(), 860),
            getToothPixmap(tooth), coords.BuccalOcclusalCrop);
    }
...
QPixmap* getOcclusal(const ToothPaintHint& tooth)
{
    auto& coords = SpriteSheets::container().getCoordinates(tooth.idx, tooth.temp);
    auto& currentTexture = SpriteSheets::container().getTexturePack(tooth.idx, tooth.temp);
