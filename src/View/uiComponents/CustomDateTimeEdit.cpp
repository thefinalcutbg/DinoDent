...
CustomDateTimeEdit::CustomDateTimeEdit(QWidget* parent, menuContent type) :
...
/* 初始化日历菜单 */
void CustomDateTimeEdit::initCalendar() {
...
/* 初始化日期导航条 */
void CustomDateTimeEdit::initHeaderWidget() {
    ...
    connect(prevYearButton, &QPushButton::clicked, this, [this]() {
        selectedYear(-1);
        }); // 上一年按钮的信号槽
    connect(nextYearButton, &QPushButton::clicked, this, [this]() {
        selectedYear(1);
        }); // 下一年按钮的信号槽
    connect(prevMonthButton, &QPushButton::clicked, this, [this]() {
    ...
    connect(calendarWidget(), &QCalendarWidget::currentPageChanged, this, [this](int year, int month) {
    ...
    connect(yearButton, &QPushButton::clicked, this, [this]() {
...
/* 设置日期标签的文本 */
void CustomDateTimeEdit::setDateLabelText(int year, int month) {
...
/* 选择月份 */
void CustomDateTimeEdit::selectedMonth(int year, int month) {
...
/* 选择年份 */
void CustomDateTimeEdit::selectedYear(int yearCount) {
    QDate validDate = calendarWidget()->selectedDate().addYears(yearCount); // 按年份加减时间,返回有效日期
    setDate(validDate);
}
/* 展示月份菜单 */
void CustomDateTimeEdit::monthMenuPopup() {
    ...
    for (int monthTableRow = 0; monthTableRow < 4; monthTableRow++) { // 月份菜单的行
        for (int monthTableColumn = 0; monthTableColumn < 4; monthTableColumn++) { // 月份菜单的列
            ...
            if (type == menuContent::MONTH) {
                emit calendarWidget()->activated(calendarWidget()->selectedDate());
            }
            else {
...
/* 展示年份菜单 */
void CustomDateTimeEdit::yearMenuPopup() {
...
/* 菜单发生变化时展示菜单的新内容 */
void CustomDateTimeEdit::menuChanged() {
    if (!yearMenu->isHidden()) {
        yearMenuPopup();
    }
...
/* 切换菜单 */
void CustomDateTimeEdit::changeMenu(int sequence) {
...
/* 过滤鼠标滚轮事件 */
bool CustomDateTimeEdit::eventFilter(QObject* widget, QEvent* event) {
...
/* 改变日历菜单类型 */
void CustomDateTimeEdit::changeType(menuContent type) {
    this->type = type;
