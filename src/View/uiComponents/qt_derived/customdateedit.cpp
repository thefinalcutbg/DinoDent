...
void CustomDateEdit::initCalendar() {
    ...
    calendarWidget()->setLocale(QLocale(QLocale::Bulgarian));
    ...
    calendarWidget()->setHeaderTextFormat(headerCharFormat);
...
void CustomDateEdit::setDateLabelText(int year, int month) {
    yearButton->setText(QStringLiteral("%1 год.").arg(year));

    QString monthStr = QLocale::system().monthName(month);
    monthStr[0] = monthStr[0].toUpper();

    monthButton->setText(monthStr);
}
/* 选择月份 */
...
void CustomDateEdit::monthMenuPopup() {
    ...
    for (int monthTableRow = 0; monthTableRow < 4; monthTableRow++) { // 月份菜单的行
        for (int monthTableColumn = 0; monthTableColumn < 4; monthTableColumn++) { // 月份菜单的列
            ...
            QString monthStr = QLocale::system().monthName(actualMonth, QLocale::FormatType::ShortFormat);
            monthStr[0] = monthStr[0].toUpper();
            monthList[monthTableCount]->setText(monthStr);
